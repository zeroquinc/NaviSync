...
def update_artist_play_counts(conn, user_id):
    """
    ...
    # Reset all artist play counts to 0 first
    cursor.execute("""
        UPDATE annotation
        SET play_count = 0
        WHERE user_id = ? AND item_type = 'artist'
    """, (user_id,))
    conn.commit()
    ...
    if has_multi_artist_table:
        # Debug: Check a sample track to understand the structure
        cursor.execute("""
            SELECT mf.id, mf.title, mf.artist, mf.artist_id, 
                   GROUP_CONCAT(mfa.artist_id) as all_artist_ids
            FROM media_file mf
            LEFT JOIN media_file_artists mfa ON mfa.media_file_id = mf.id
            WHERE mf.artist LIKE '%,%'
            GROUP BY mf.id
            LIMIT 1
        """)
        sample = cursor.fetchone()
        if sample:
            print(f"🔍 Debug - Sample multi-artist track:")
            print(f"   Track: {sample[1]} by {sample[2]}")
            print(f"   Primary artist_id: {sample[3]}")
            print(f"   All artist_ids in media_file_artists: {sample[4]}")
        
        # Use media_file_artists table, filtering by role='artist' to exclude albumartist
        # This prevents counting album artists and only counts actual track artists
        cursor.execute("""
            SELECT 
                mfa.artist_id,
...
def update_album_play_counts(conn, user_id):
    """
    ...
    # Reset all album play counts to 0 first
    cursor.execute("""
        UPDATE annotation
        SET play_count = 0
        WHERE user_id = ? AND item_type = 'album'
    """, (user_id,))
    conn.commit()
    
    # Get all albums and their total play counts from tracks
    cursor.execute("""
        SELECT 
            mf.album_id,
            SUM(COALESCE(a.play_count, 0)) as total_plays
